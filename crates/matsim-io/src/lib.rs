...
mod tests {
    ...
    fn loads_multiple_plans_and_replans_best_score() {
        ...
        assert_eq!(scenario.population.persons.len(), 2);
        ...
        assert_eq!(scenario.population.persons[1].plans.len(), 2);
        assert_eq!(scenario.population.persons[1].plans[0].score, Some(10.0));
        assert_eq!(scenario.population.persons[1].plans[1].score, Some(200.0));
        ...
        assert!((output.iterations[0].score_stats.avg_executed - 83.341_925).abs() < 1.0e-6);
        assert!((output.iterations[1].score_stats.avg_executed - 96.478_115).abs() < 1.0e-6);
        assert!((output.iterations[1].score_stats.avg_worst - 83.341_925).abs() < 1.0e-6);
    ...
    fn mixed_fixture_uses_best_score_and_reroute_together() {
        ...
        assert!((output.iterations[1].score_stats.avg_executed - 93.651_611).abs() < 1.0e-6);
    ...
    #[test]
    fn explains_person_plans_for_two_plan_fixture() {
        let root = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
            .join("../../..")
            .join("matsim-rs/examples/two-plans/config.xml");
        let scenario = load_scenario(&root).unwrap();

        let explanation = explain_person_plans(&scenario, "1").unwrap();
        assert_eq!(explanation.selected_plan_index, 0);
        assert_eq!(explanation.plans.len(), 2);
        assert_eq!(explanation.plans[0].score, Some(1.0));
    ...
    fn loads_events_csv_grouped_by_iteration() {
        ...
        fs::write(
            ...
            concat!(
                ...
                "0;2.000000;alice;link_leave;mid;0\n",
        ...
        assert_eq!(grouped[0].1[1].event_type, "link_leave");
        assert_eq!(grouped[0].1[1].link_id.as_deref(), Some("mid"));
