...
use matsim_core::{
    ...
    write_node_runstats, write_node_selector_previewstats, write_node_selector_windowstats,
    write_node_step_batchstats, write_outputs,
};
...
fn run_command(config_path: &Path) -> Result<(), CliError> {
    ...
    write_node_crossingstats(
        &output_dir.join("node_crossingstats.csv"),
        &output,
        &scenario.network,
    )?;
    write_node_prioritystats(
        &output_dir.join("node_prioritystats.csv"),
        &output,
        &scenario.network,
    )?;
    ...
    write_node_step_batchstats(
        &output_dir.join("node_step_batchstats.csv"),
        &output,
        &scenario.network,
    )?;
    write_node_selector_previewstats(
        &output_dir.join("node_selector_previewstats.csv"),
        &output,
        &scenario.network,
    )?;
    write_node_selector_windowstats(
        &output_dir.join("node_selector_windowstats.csv"),
        &output,
        &scenario.network,
    )?;
    write_population(
